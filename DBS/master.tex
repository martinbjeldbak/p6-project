\documentclass[a4paper, 12pt, twoside, article]{memoir}
\synctex=1

\counterwithout{section}{chapter}

\usepackage{cleveref}
\usepackage[utf8x]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[english]{babel}

\usepackage[pdftex]{graphicx}
\usepackage{epstopdf}

\usepackage[activate={true,nocompatibility},final,tracking=true,kerning=true,spacing=true,factor=1100,stretch=10,shrink=10,verbose=silent]{microtype}

\captiontitlefont{\itshape}
\usepackage{tikz}
\usepackage[top=1in,bottom=1in,left=1.2in,right=1.2in]{geometry}

\usepackage{amsmath}
\usepackage{siunitx}

% Smaller spacing between lists
\usepackage{enumitem}
\setlist[itemize]{noitemsep}
\setlist[enumerate]{noitemsep}
\usepackage{listings}

%\usepackage{minted}
\usepackage{hyperref}
\usepackage[nodayofweek]{datetime}
\newcommand{\formatdatewday}[3]{\dayofweekname{#1}{#2}{#3} \formatdate{#1}{#2}{#3}}
\newcommand{\deadline}[3]{Deadline: \formatdatewday{#1}{#2}{#3}}

\newcommand{\lag}{\langle}
\newcommand{\rag}{\rangle}
\newcommand{\setof}[2]{\ensuremath{\{ #1 \mid #2 \}}}
\newcommand{\set}[1]{\ensuremath{\{ #1 \}}}
\newcommand{\besk}[1]{\ensuremath{\lag #1 \rag}}
\newcommand{\ra}{\rightarrow}
\newcommand{\lra}{\longrightarrow}
\newcommand{\Ra}{\Rightarrow}
\newcommand{\var}[1]{\text{ #1 }}
\newcommand{\dvar}[1]{\lag\text{ #1 }\rag}

\newcommand{\key}[1]{\ensuremath{\underline{\mathit{#1}}}}
\newcommand{\schema}[1]{\set{[#1]}}
\newcommand{\refs}[2]{#1 \ra #2}

\setlength{\parindent}{0pt}
\nonzeroparskip{}

\title{Database Systems, spring 2014\\Mini Project}
%\date{\protect\formatdate{6}{2}{2014} to \protect\formatdate{6}{2}{2014}}
\date{%
  \texttt{\href{mailto:d601f14@cs.aau.dk}{d601f14}, 1.1.01}\\
  \protect\formatdate{6}{2}{2014} to \today %\protect\formatdate{6}{2}{2014}
}

\author{%
  Elias Obeid\\
  \href{mailto:eobeid11@student.aau.dk}{\texttt{eobeid11@student.aau.dk}}
  \and
  Kent Caspersen\\
  \href{mailto:kcaspe11@student.aau.dk}{\texttt{kcaspe11@student.aau.dk}}
  \and
  Martin Madsen\\
  \href{mailto:mbma11@student.aau.dk}{\texttt{mbma11@student.aau.dk}}
}

\begin{document}

\frontmatter

\maketitle
\thispagestyle{empty}
\pagebreak

\mainmatter

\input{1-06.02.14/root}
\input{2-17.02.14/root}
\input{3-12.03.14/root}
\input{4-18.03.14/root}
\input{5-31.03.14/root}

%\newpage
%\appendix
%\chapter{SQL: Preliminary Database Modeling}\label{sec:sqlPre}
%\inputminted{sql}{1-06.02.14/selfStudy1db.sql}

\end{document}
