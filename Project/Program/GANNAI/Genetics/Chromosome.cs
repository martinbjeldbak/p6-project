...
namespace Genetics {
  public class Chromosome {
    ...
    public double CalcDouble(int from, int to)
    {
        ...
        //if value is only encoded by 1 bit
        if (to == from)
            return Bitstring[from] ? 5 : 0;

        //find out how many bits are used in the increasing sequence of powers of two
        int aggregateBits = to - from; //If length is only 1 bit, that bit will be used for both negation and aggregation
