...
This is because we use the sigmoid activation function, for which the value $f(x)$ changes only a little when $x \leq -5$ and $x  \geq 5$. If a weight or bias of a neural network is encoded by the bit string $b_1 b_2 \cdots b_u$, we decode its real value using the formula
\[
w = 5\frac{b_1(1-2b_u)2^0 b_2(1-2b_u)2^1 \cdots b_{u-1}(1-2b_u)2^{u-2}}{2^u-1}
\]
which is a simple binary encoding where the most significant bit negates the value if set, and the value is normalized to the range $[-5, 5]$. 
