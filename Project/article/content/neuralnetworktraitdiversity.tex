...
We represent the number of species in binary, and the $i$th bit is set to 1 if 
\[\forall o \in O\; \lvert\; o_i \geq o\]
where $O$ contains all output neurons, and $o$ is a single output neuron, then the result of $o_i$ is greater than or equal to any other output neuron's result, given input $R_i$.
