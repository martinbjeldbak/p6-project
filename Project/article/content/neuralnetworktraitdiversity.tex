...
In a population where each individual has $b$ output neurons, the total number of available species is $2^b - 1$.  As an example; assume that we have a neural network with six output neurons. If the second output neuron has the highest output, then that neural network is placed in species $S_2$, because the toggled bits correspond to \mathtt{000010}. If another individual has multiple output neurons giving the highest output, say the first and third output neuron, then that neural network will be placed in species $S_5$, because the toggled bits correspond to \mathtt{000101}. If a third neural network has all of its output neurons give the same result, then that neural network is placed in $S_{63}$, because the toggled bits correspond to \mathtt{111111}. Any species that is empty, does not count as a species in the diversity measurement.

% This means that for any neural network belonging to species $S_k(R_i)$, the bits in $k$, which are 1 are the output neurons with greater than or equal the value of any other output neuron given input $R_i$.% This definition implies that the number of species equals the number of output neurons $b$, e.g.\ $1 \leq k \leq b$. We distribute neural networks into a set of species as follows
...
The diversity is calculated $m$ times with \dia{} as
%
