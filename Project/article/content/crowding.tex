...
If $d\left(\generation{i}\right) < \alpha$, replace the $\tfrac{n}{2}$ least fit individuals from \generation{i} by random immigrants, where $n = \generationl{i}$. Use the \emph{Single Parent Elitism} replacement rule on $\left(\generation{i}, \offspring{i}\right)$.
