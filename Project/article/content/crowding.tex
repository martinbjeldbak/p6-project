...
\label{sec:crowding} 
A simple way to compose the $i$th generation, denoted $G_i$, is by selecting the $n$ best individuals from the union of $G_{i-1}$ and $O_{i-1}$, where $O_{i-1}$ contains the offspring produced from $G_{i-1}$. %by selecting a number of individuals, where more fit individuals are more likely to be selected.
